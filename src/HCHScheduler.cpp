...
public:
    set<int> rqID;
    HCHScheduler() { need_schedule.clear(); }
    ~HCHScheduler() {}
...
extern "C"
{
    HCHScheduler *C_new() { return new HCHScheduler(); }
    void C_gc(HCHScheduler *self)
    {
        if (self == nullptr)
        {
            cerr << "CTMD\n";
    ...
    void C_init(HCHScheduler *self, int driver_num) { return self->C_init(driver_num); }
    Result *C_schedule(HCHScheduler *self, int logical_clock, Request *request_list, int len_request, Driver *driver_list, int len_driver)
    {
